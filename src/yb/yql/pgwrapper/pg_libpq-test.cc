...
Result<master::TabletLocationsPB> GetTablegroupTabletLocations(
  ...
  bool exists = VERIFY_RESULT(client->TablegroupExists(database_name, tablegroup_name));
  if (!exists) {
  ...
  if (tg_id.empty()) {
    return STATUS(NotFound, "tablegroup not found");
  }
  ...
  RETURN_NOT_OK(WaitFor(
      [&]() -> Result<bool> {
        Status s = client->GetTabletsFromTableId(
            tg_id + master::kTablegroupParentTableIdSuffix,
            0 /* max_tablets */,
            &tablets);
        if (s.ok()) {
          return tablets.size() == 1;
